...
class Solution(object):
    def romanToInt(self, s):
        ...
        sum_ = 0
        if(not s) : return sum_

        m = len(s)
        #I < V
        if(m > 1 and roman_to_int[s[-1]] > roman_to_int[s[-2]] ) : #IV
            sum_ += (roman_to_int[s[-1]] - roman_to_int[s[-2]]) + self.romanToInt(s[:-2])
        else :
            #Pick only subtring
            sum_ += roman_to_int[s[-1]] + self.romanToInt(s[:-1])
        return sum_
